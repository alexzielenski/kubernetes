				...
				if err != nil {
					...
					for i, cr := range compResults {
						...
						if cr.UsesOldSelf {
							...
							allErrs.CELErrors = append(allErrs.CELErrors, field.Invalid(fldPath.Child("x-kubernetes-validations").Index(i).Child("optionalOldSelf"), schema.XValidations[i].Rule, "may not be set if oldSelf is not used in rule"))
						}
					}
				}
			}
